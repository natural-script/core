...
import * as inherit_utils from "./inheritance";
...
export default class Brain {
	...
	/**
	reply (string user, string msg[, scope])
	...
	*/
	reply(user, msg, scope) {
		...
		self.master._session.set(user, {
			__initialmatch__: null
		});

		// If the BEGIN block exists, consult it first.
		if (self.master._topics.__begin__) {
			let begin = (self._getReply(user, "request", "begin", 0, scope));

			// OK to continue?
			if (begin.indexOf("{ok}") > -1) {
				reply = (self._getReply(user, msg, "normal", 0, scope));
				begin = begin.replace(/\{ok\}/g, reply);
			}

			reply = (self.processTags(user, msg, begin, [], [], 0, scope));
		} else {
			reply = (self._getReply(user, msg, "normal", 0, scope));
		}

		// Save their reply history
		let history = (self.master._session.get(user, "__history__"));
	...
	/**
	_getReply (string user, string msg, string context, int step, scope)
	...
	*/
	_getReply(user, msg, context, step, scope) {
			...
			for (let j = 0, len = allTopics.length; j < len; j++) {
				...
				if (self.master._sorted.thats[top].length) {
					...
					for (let k = 0, len1 = self.master._sorted.thats[top].length; k < len1; k++) {
						...
						// Match?
						let match = lastReply.match(new RegExp(`^${botside}$`, 'mi'));
						if (match) {
							...
							if (isAtomic) {
								...
								let match = msg.match(new RegExp(`^${regexp}$`, 'mi'));
								if (match) {
									isMatch = true;
									// Get the stars
									stars = match;
		...
		self.master._session.set(user, {__lastmatch__: matchedTrigger});
		let lastTriggers = [];
		if (step === 0) {
			self.master._session.set(user, {
		...
		if (matched) {
			...
			self.master._session.set(user, {__last_triggers__: lastTriggers});
			...
			for (let n = 0; n < 1; n++) {
				...
				if (matched.redirect != null) {
					...
					let redirect = (self.processTags(user, msg, matched.redirect, stars, thatstars, step, scope));

					self.say(`Pretend user said: ${redirect}`);
					reply = (self._getReply(user, redirect, context, step + 1, scope));
					break;
				}
				...
				for (let o = 0, len4 = matched.condition.length; o < len4; o++) {
					...
					if (halves && halves.length === 2) {
						...
						if (condition) {
							...
							left = (self.processTags(user, msg, left, stars, thatstars, step, scope));
							right = (self.processTags(user, msg, right, stars, thatstars, step, scope));
	...
	*/
	...
	/**
	triggerRegexp (string user, string trigger)
	...
	*/
	triggerRegexp(user, regexp) {
		...
		regexp = regexp.replace(/<line break>/g, "<<br>>"); // Convert <line break> into \n
		regexp = regexp.replace(/<(\d+) word(?:s|)>/g, (match, p1, offset, string) => {
			let wordsRegex = [];
			for (const i of Array(parseInt(p1))) {
				wordsRegex.push('\\S+');
			}
			return wordsRegex.join(' ');
		});
	...
	processTags(user, msg, reply, st, bst, step, scope) {
		...
		while (true) {
			...
			if (tag === "bot" || tag === "env") {
				...
				let existingValue = (self.master.getUservar(user, name));
		...
		while (match) {
			...
			let subreply = (self._getReply(user, target, "normal", step + 1, scope));
