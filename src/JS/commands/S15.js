...
export default function({
  ...
  if (
    dbType.findBestMatch(
      window.wordsTranslationsDB.Words["publicC"][declarations.langCode]
    ).rating > 0.8
  ) {
    ...
    return firebase
      .database()
      .ref("public/" + dbName + "/" + branchName + "/" + newPostKey)
      .set(parsedData);
  } else if (
    dbType.findBestMatch(
      window.wordsTranslationsDB.Words["privateC"][declarations.langCode]
    ).rating > 0.8
  ) {
    const newPostKey = firebase
      .database()
      .ref("private/" + window.NS.user.uid + "/" + dbName)
      .child(branchName)
      .push().key;
    return firebase
      .database()
