...
import { conditionsDB } from "core/analyzer";
...
export default function parseCondition(statement) {
	...
	for (let subCondition of conditions) {
		subCondition = subCondition.replace(XRegExp(`(?<=${conditionsRegex}), (?=${conditionsRegex})`, "gmi"), " && ").replace(XRegExp(`(?<=${conditionsRegex}) ${getTranslations(
            "or"
          )} (?=${conditionsRegex})`, "gmi"), " || ");
		subCondition = XRegExp.replace(subCondition, XRegExp(`${conditionRegexPrefix}(?:(?<condition>${conditionsDB.join(
          "|"
        )}))${conditionRegexSuffix}`, "gmin"), match => {
			...
			if (/^Co(1|2|3|4)$/.test(data.id)) {
				...
				return `theComponentCalled(\`${
            data.elementName
          }\`).getVal() ${operator} parseStringValue(\`${
            data.value
